...
import {
  CropSpecies,
  CropSpeciesSewMethods,
} from "@functions/gql/crop-species/CropSpecies.entity";
...
export class GeoLocationService extends BaseService(GeoLocation) {
  ...
  async importTest() {
    ...
    const datas = JSON.parse(
      fs
        .readFileSync(
          "/home/user1a/WebstormProjects/gardenplanner-lambda/data/city/export.json"
        )
        .toString()
    );
    /*const datas = [];
    ...
    datas.forEach((cityData) => {
      ...
      ]; // { type: "Point", coordinates: [[ cityData.lat, cityData.lng]]}
    ...
    for (const geoLocation of geoLocationColl) {
      ...
      if (!cityData) {
        throw new Error(
          "Could not find a cityData with importId matching: " +
            cityData.importId
        );
      }
      if (cityData.nearestMatch) {
        ...
        if (!found) {
          throw new Error(
            "Could not find a nearestMatch with importId matching: " +
              cityData.nearestMatch.id
          );
        }
  ...
  async getCropSpecieDataByGeoLocation(
    ctx,
    input: CropSpecieDataByGeoLocationInput
  ) {
    const baseGeoLocation = await this.geoLocationModel.findOne({
      _id: input.geoLocationId,
    });
    if (!baseGeoLocation) {
      return null;
    }
    ...
    if (baseGeoLocation.nearestMatchGeoLocationId) {
      geoLocation = await this.geoLocationModel.findOne({
        _id: baseGeoLocation.nearestMatchGeoLocationId,
      });
      if (!geoLocation) {
        throw new Error(
          "Could not find GeoLocation" +
            baseGeoLocation.nearestMatchGeoLocationId +
            " referenced by " +
            baseGeoLocation._id
        );
      }
  ...
  parseCrops(geoLocation: GeoLocation, crops: CropSpecies[]) {
    ...
    crops.forEach((crop) => {
      ...
      for (let i = 1; i < 25; i++) {
        ...
        if (crop.lowTemp > avgTemp || crop.highTemp < avgTemp) {
          // Fail
